...
// Payment state lives in ./pay.mjs so it can be unit-tested in node without
// React. Extension is required: webpack does not resolve bare .mjs imports.
import {
  paymentLabel,
  paymentLabelVariant,
  isCodTagged,
...
// pay.mjs’s payment end-states, so isPaymentHeld correctly says "not awaiting
...
export default function ShopifyOrdersDashboard() {
  ...
  //
  // Scoped to readyOrders, not all orders: unsettled states (unconfirmed
  // PENDING) now live on Needs Review, so offering them here would be a filter
