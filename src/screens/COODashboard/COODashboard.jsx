...
export default function COODashboard() {
    ...
    // rendered inside the Date dropdown panel.
    const {
    ...
    const brandStats = useMemo(() => {
        const prevDr = comparisonPeriodRange(periodRangeValue, comparison);
        // `current`/`prev` are SALES: the order counts and channel counts below
        // were running over every row, so 0-value stock movements and
        // alterations inflated them. currentAll keeps the full set for the
        // refund figures, which count rows isSaleOrder excludes.
        const currentAll = orders.filter(o => inPeriod(o.created_at));
        const current = currentAll.filter(isSaleOrder);
        const prev = prevDr ? orders.filter(o => inRange(prevDr, o.created_at) && isSaleOrder(o)) : [];

        const totalRevenue = current.reduce((s, o) => s + Number(o.net_total ?? o.grand_total_after_discount ?? o.grand_total ?? 0), 0);
        const netSbRev = totalNetSbRevenue(current);
        const prevRevenue = prev.reduce((s, o) => s + Number(o.net_total ?? o.grand_total_after_discount ?? o.grand_total ?? 0), 0);
        const totalRefund = currentAll.filter(o => o.refund_reason).reduce((s, o) => s + Number(o.net_total ?? o.grand_total_after_discount ?? o.grand_total ?? 0), 0);
        const refundCount = currentAll.filter(o => o.refund_reason).length;
    ...
    const consignmentStats = useMemo(() => {
        ...
        const consignmentOrders = orders.filter(o => o.b2b_order_type === "Consignment" && inPeriod(o.created_at));

        // Dispatch daily/weekly/monthly
        const dispatched = orders.filter(o => o.dispatched_at && o.b2b_order_type === "Consignment" && inPeriod(o.created_at));
    ...
    }, [consignmentInventory, vendors, orders, inPeriod]);
    ...
    }, [products, consignmentInventory, inventorySearch, inventoryStockFilter, inventoryTypeFilter, inventoryPage, variantInventory]);
    ...
    const appliedFilters = useMemo(() => { const c = []; if (ordersTimeline !== "all") c.push({ type: "date", label: periodLabel(ordersTimeline) }); if (filters.minPrice > 0 || filters.maxPrice < 500000) c.push({ type: "price", label: `\u20B9${(filters.minPrice / 1000).toFixed(0)}K - \u20B9${(filters.maxPrice / 1000).toFixed(0)}K` }); filters.payment.forEach(p => c.push({ type: "payment", value: p, label: p.charAt(0).toUpperCase() + p.slice(1) })); filters.priority.forEach(p => c.push({ type: "priority", value: p, label: p.charAt(0).toUpperCase() + p.slice(1) })); filters.store.forEach(s => c.push({ type: "store", value: s, label: s })); if (filters.salesperson) c.push({ type: "salesperson", label: filters.salesperson }); return c; }, [filters, ordersTimeline]);
    const removeFilter = (type, value) => { if (type === "date") clearOrdersPeriod(); else if (type === "price") setFilters(p => ({ ...p, minPrice: 0, maxPrice: 500000 })); else if (type === "salesperson") setFilters(p => ({ ...p, salesperson: "" })); else setFilters(p => ({ ...p, [type]: p[type].filter(v => v !== value) })); };
    const clearAllFilters = () => { setFilters({ minPrice: 0, maxPrice: 500000, payment: [], priority: [], orderType: [], store: [], salesperson: "" }); clearOrdersPeriod(); };
    ...
    return (
        <div className="admin-page">
            ...
            <DashboardHeader
                title="COO Dashboard"
                onHome={() => setActiveTab("operations")}
            ...
            <div className="admin-layout">
                ...
                <main className="admin-main">
                    {/* Timeline bar (shared PeriodFilter; comparison select rides in its right slot) */}
                    ...
                    {activeTab === "inventory" && (
                        <div>
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", marginBottom: 12 }}>
                                <div className="admin-search-wrapper" style={{ maxWidth: 300 }}><input type="text" placeholder="Search product or SKU..." value={inventorySearch} onChange={(e) => setInventorySearch(e.target.value)} className="admin-search-input" /></div>
                            ...
                            <div className="admin-table-wrapper"><div className="admin-table-container"><table className="admin-table">
                                <thead><tr><th>Product</th><th>SKU</th><th>Type</th><th>Stock</th></tr></thead>
                                <tbody>{inventoryStats.currentProducts.length === 0 ? <tr><td colSpan="4" className="no-data">{products.length === 0 ? "No products yet" : "No products match the search/filters"}</td></tr> : inventoryStats.currentProducts.map(p => { const qty = p.sync_enabled ? getLxrtsTotalInventory(p.id) : (p.inventory || 0); return (<tr key={p.id}><td>{p.name || "-"}</td><td>{p.sku_id || "-"}</td><td>{p.sync_enabled ? "LXRTS" : (p.inventory === 9999 ? "MTO" : "Regular")}</td><td><span className={qty === 0 ? "admin-stock-out" : qty < 5 ? "admin-stock-low" : "admin-stock-ok"}>{qty === 9999 ? "MTO" : qty}</span></td></tr>); })}</tbody>
                            </table></div></div>
                    ...
                    {activeTab === "orders" && (
                        <div className="admin-orders-tab">
                            ...
                            <div className="admin-filter-bar" ref={dropdownRef}>
                                <div className="filter-dropdown"><button className={`filter-btn ${ordersTimeline !== "all" ? "active" : ""}`} onClick={() => setOpenDropdown(openDropdown === "date" ? null : "date")}>Date {"\u25BE"}</button>{openDropdown === "date" && (<div className="dropdown-panel"><div className="dropdown-title">Select Period</div><PeriodFilter {...ordersPeriodProps} variant="select" label="Order date:" /><button className="dropdown-apply" onClick={() => setOpenDropdown(null)}>Apply</button></div>)}</div>
