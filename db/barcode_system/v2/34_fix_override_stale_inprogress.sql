-- ============================================================
-- 34. Fix: PM manual override leaves a stale in-progress ledger row, so the
--     next scan is wrongly blocked with "X is still In-Progress".
--
-- BUG: advance_component_stage's manual_override branch moves current_stage
-- correctly but does NOT close every open component_stage_progress row. For a
-- _completed target it calls reconcile_ledger_to_step(v_to_step), which SKIPS
-- skippable steps (reconcile_ledger_to_step, file 27:61; under file 31 every
-- step but Cloth Issue is skippable). So a stage left 'in_progress' by an
-- earlier scan is never closed, and the next scan hits the open-stage guard
-- (any_open_stage_except, file 31:166) -> "Pattern Cutting is still In-Progress",
-- etc. Confirmed intent: an override TO a _completed stage should leave it
-- fully done — nothing open.
--
-- FIX: at the top of the manual_override branch, close every 'in_progress'
-- ledger row that is NOT the new target step. The target's own row is then
-- handled by the existing branch logic (opened in_progress for an _in_progress
-- target — legitimately "being worked", scan it out later; or written completed
-- for a _completed target). Net: override -> _completed leaves nothing open;
-- override -> _in_progress leaves ONLY that one target open.
--
-- Redeploys ONLY advance_component_stage (the bug is isolated to it; its helper
-- functions any_open_stage_except / reconcile_ledger_to_step / is_step_skippable
-- from file 31 are unchanged). Idempotent. Run on uat first, then prod.
-- ============================================================
...
BEGIN
  ...
  IF v_component.current_stage IN ('disposed', 'scrapped') THEN
    RETURN jsonb_build_object('success', false, 'error', 'COMPONENT_TERMINATED',
      'message', 'Component has been ' || v_component.current_stage::text, 'barcode', p_barcode);
  END IF;
