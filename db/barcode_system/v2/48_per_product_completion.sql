-- ============================================================
-- 48. Per-product completion — the final completed/dispatched/delivered model.
--
-- Consolidates the 44/45/46 back-and-forth into the agreed model:
--
--   final_qc_passed -> [Mark as Completed] -> production_complete
--                   -> [Packaging & Dispatch scan] -> dispatched
--                   -> [SA / courier] -> delivered
--
-- COMPONENT level: Mark as Completed moves the chosen product's
--   final_qc_passed pieces to production_complete. Their badge reads
--   "Completed". Packaging & Dispatch REQUIRES production_complete now — a
--   piece still at final_qc_passed cannot be packaged until it is marked
--   completed (the human gate the client asked for).
--
-- ORDER level: orders.status becomes 'completed' only when EVERY active piece
--   is at production_complete or beyond (least-advanced-product rule).
--   Completing product 1 of a 3-product order leaves the ORDER in production;
--   its status flips to completed when all three are done.
--
-- Final QC stays mandatory and is checked PER PRODUCT (p_item_index), so one
-- finished product can be completed while others are still in production.
--
-- WHERE TO RUN:
--   * uat  - had 44/45 (production_complete existed) then 46 (removed it). This
--            restores it and is the current definition of all four functions.
--            STEP 0 re-parks anything 46 pushed back to final_qc_passed? No -
--            46 moved production_complete -> final_qc_passed, which is the
--            correct pre-completion state, so nothing to undo. Just run B-F.
--   * prod - 44/45/46 never ran. This is the ONLY migration to run; it deploys
--            the whole model. (46's status default + Pending backfill are
--            included below so prod gets them too.)
--
-- Idempotent (CREATE OR REPLACE / IF NOT EXISTS). Safe to re-run.
--
-- RUN SECTION A ALONE FIRST (enum add cannot share a txn with the functions).
-- ============================================================
...
-- -- SECTION B - status column default + Pending backfill (from 46; harmless
-- to re-run). Stops new B2B orders defaulting to 'Pending', and normalises the
-- existing ones. Only touches non-terminal rows; never rewrites delivered/
-- completed/cancelled.
...
-- -- SECTION C - stage model with production_complete.
...
-- -- SECTION D - Final QC mandatory (accepts final_qc_passed / production_complete).
...
-- -- SECTION E - advance_component_stage: 45's body verbatim (auto-activation,
-- cloth-issue ledger fix, dispatched terminal guard). production_complete is
-- set only by manual_complete_order, never a scan.
...
-- -- SECTION F - packaging REQUIRES production_complete (not final_qc_passed).
...
BEGIN
  ...
  SELECT NOT EXISTS (
    SELECT 1 FROM order_components
    WHERE order_id = p_order_id AND is_active = TRUE
      AND current_stage NOT IN ('disposed','scrapped','production_complete','packaging_dispatch','dispatched')
      AND (p_item_index IS NULL OR item_index = p_item_index)
  ...
  IF NOT v_all_qc_passed THEN
    RETURN jsonb_build_object('success', false, 'error', 'NOT_ALL_QC_PASSED',
      'message', CASE WHEN p_item_index IS NULL
                      THEN 'Not all components are marked Completed yet (Mark as Completed first)'
                      ELSE 'Not all of product ' || (p_item_index + 1) || '''s components are marked Completed yet'
                 END);
  END IF;
...
-- -- SECTION G - manual_complete_order: per-product completion.
...
BEGIN
  ...
  -- A piece is completable once it is at final_qc_passed (or already past it).
  -- Anything earlier blocks — listed back so the UI can show which pieces.
  FOR r IN
  ...
  -- ---- Move the scoped product's final_qc_passed pieces to production_complete ----
  -- This is what turns the component badge to "Completed" and lets Packaging &
  -- Dispatch pick the piece up (packaging requires production_complete).
  FOR r IN
  ...
  -- production_complete (least-advanced-product rule). Completing one product
  -- of a multi-product order leaves the ORDER in production. ----
  SELECT EXISTS (
    SELECT 1 FROM order_components
    WHERE order_id = p_order_id AND is_active = TRUE
...
-- VERIFY
-- ------------------------------------------------------------
-- 1) enum has it:  SELECT get_stage_step('production_complete');  -- 10
-- 2) default fixed: SELECT column_default FROM information_schema.columns
--    WHERE table_name='orders' AND column_name='status';  -- 'order_received'
-- 3) no Pending left: SELECT count(*) FROM orders WHERE lower(status)='pending';  -- 0
-- 4) dispatched terminal guard intact:
--    SELECT substring(prosrc from 'IN \(''disposed''[^)]*\)')
--    FROM pg_proc WHERE proname='advance_component_stage';  -- lists 'dispatched'
-- 5) Functional (uat):
--    a. Multi-product order, product 1 all final_qc_passed, product 2 mid-prod
--       -> Mark as Completed for PRODUCT 1: product 1's pieces go
--       production_complete (badge "Completed"), ORDER stays in production.
--    b. Product 1 then packaging-scans fine; a product-2 piece still at, say,
--       stitching CANNOT be packaged.
--    c. A product whose pieces are NOT all final_qc_passed -> Mark as Completed
--       refused, listing the blocking pieces.
--    d. Complete every product -> orders.status = 'completed'.
