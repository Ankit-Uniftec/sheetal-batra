...
// ============================================================
// scan-report-daily — WhatsApp the day's scan report as an Excel-openable CSV.
//
...
// CSV as the Production Manager's "Scan Report" export, uploads it to the
// public `reports` storage bucket, and sends it through the spur-whatsapp
// function using the daily_scan_report template — whose Document header makes
// the file appear IN the chat as an attachment, not a link.
//
// Invocation:
...
serve(async (req) => {
  try {
    ...
    // ---- 2) Build the CSV (same columns as the dashboard export) ----
    ...
    const filename = `scan_report_${day}.csv`;
