...
- **Order-number format is `SB-<CHANNEL>-MMYY-NNNNNN`** (e.g. `SB-DLC-0726-003625`); the **prefix is the authoritative channel signal** — `DLC`=Delhi, `LDHC`=Ludhiana, `B2B`, `COM`=Comms, `PVT`=Private, `EXB`=Exhibition, `SHOPIFY`=website orders, `STOCK`=retail internal stock, `B2BSTOCK`=B2B internal stock. (`SHOP` is a retained legacy alias for website orders minted before the rename — see `CHANNEL_BY_ORDER_PREFIX`.) The store code may be any length but must contain **no dash**: the whole app parses `order_no` by dash position, never by character offset. Flags (`is_b2b`, `is_gifting`, `is_stock_order`, `is_alteration`, LXRTS/`sync_enabled` = Shopify-synced) modify behavior.
